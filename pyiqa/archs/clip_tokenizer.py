"""Vendored CLIP tokenizer.

Adapted from the openai/CLIP repository (MIT licence).
The original clip/simple_tokenizer.py is reproduced verbatim below; the
tokenize() helper from clip/clip.py follows with the pkg_resources version
checks removed (pyproject.toml already requires torch>=1.13, which is above
every threshold those checks guarded against).
"""
...
# ---------------------------------------------------------------------------
# SimpleTokenizer  (from clip/simple_tokenizer.py)
# ---------------------------------------------------------------------------
...
def bytes_to_unicode():
    ...
    utf-8 bytes and unicode strings. And avoids mapping to
...
# ---------------------------------------------------------------------------
# tokenize  (from clip/clip.py, pkg_resources version checks removed)
# ---------------------------------------------------------------------------
...
def tokenize(
    ...
    -------
    torch.Tensor
        A 2-D tensor containing the resulting tokens,
    ...
    # torch>=1.13 is required by pyproject.toml; dtype=torch.int is always correct.
